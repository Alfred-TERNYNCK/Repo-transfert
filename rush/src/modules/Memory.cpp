...
Krell::Memory::Memory() : AModule<MemoryData>("RAM")
{

}

void Krell::Memory::refreshData()
{
    struct sysinfo info{};
...
void Krell::Memory::plotMemoryGraphFor(std::vector<double> &memoryUsageGraph, double toPlot) const
{
...
void Krell::Memory::drawText(ModuleBox modBox)
{
...
void Krell::Memory::drawGraphical(sf::RenderWindow &window, const sf::Font &font, ModuleBox box)
{
    ...
    rectangle->setOrigin(bounds.left + bounds.width / 2.0f, bounds.top + bounds.height / 2.0f);
    ...
    memory->setOrigin(bounds.left + bounds.width / 2.0f, bounds.top + bounds.height / 2.0f);
...
void Krell::Memory::drawGraphicLines(sf::RenderWindow &window, ModuleBox &box)
{
    int i = 0;
    (void) box;
    ...
    for (double d: data.physicalMemoryUsageGraph) {
        ...
        line->setOrigin(lineBounds.left + lineBounds.width / 2.0f, lineBounds.top + lineBounds.height);
...
int Krell::Memory::getId() const
{
    return 1;
}

std::string Krell::Memory::getName() const
{
    return "RAM";
}
